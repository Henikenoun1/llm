...
LOG_FILE="$LOGS_DIR/lancer_${timestamp}.log"
LATEST_LINK="$LOGS_DIR/latest_train.log"
...
printf -v TRAIN_LOOP 'stdbuf -oL -eL bash train_all.sh %s2>&1 | tee %q' "$EXTRA_ARGS_ESCAPED" "$LOG_FILE"
...
if pgrep -af 'scripts/run_axolotl_full_pipeline.py|scripts/prepare_axolotl_pipeline.py|scripts/axolotl_cli.py' >/dev/null; then
  echo "another training pipeline is already running"
  pgrep -af 'scripts/run_axolotl_full_pipeline.py|scripts/prepare_axolotl_pipeline.py|scripts/axolotl_cli.py'
  exit 1
fi
